...
namespace AHRS
{
    ...
    public class MahonyAHRS
    {
        ...
        public void Update(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, float time)
        {
            ...
            float time_duration;
            time_duration = (float)0.005;

            // Integrate rate of change of quaternion
            pa = q2;
            pb = q3;
            pc = q4;
            q1 = q1 + (-q2 * gx - q3 * gy - q4 * gz) * (0.5f * time_duration);
            q2 = pa + (q1 * gx + pb * gz - pc * gy) * (0.5f * time_duration);
            q3 = pb + (q1 * gy - pa * gz + pc * gx) * (0.5f * time_duration);
            q4 = pc + (q1 * gz + pa * gy - pb * gx) * (0.5f * time_duration);
