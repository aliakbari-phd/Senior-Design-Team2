...
namespace Microsoft.Samples.Kinect.DiscreteGestureBasics
{
    ...
    /// </summary>
    public sealed class ZeroResultView : INotifyPropertyChanged
    {
        /// <summary> Image to show when the 'detected' property is true for a tracked body </summary>
        private readonly ImageSource zeroImage = new BitmapImage(new Uri(@"Images\0.png", UriKind.Relative));

        /// <summary> Image to show when the 'detected' property is false for a tracked body </summary>
        private readonly ImageSource notZeroImage = new BitmapImage(new Uri(@"Images\not0.png", UriKind.Relative));
        ...
        private double confidence = 0.0f;
        ...
        public ZeroResultView(int bodyIndex, bool isTracked, bool detected, double confidence, KinectBodyView kinectBodyView)
        {
        ...
        /// <summary> 
        /// Gets a double value which indicates the detector's confidence that the gesture is occurring for the associated body 
        /// </summary>
        ...
        public void UpdateZeroResult(bool isBodyTrackingIdValid, bool isGestureDetected, double detectionConfidence, KinectBodyView kinectBodyView)
        {
            ...
            else
            {
                ...
                //this.BodyColor = this.trackedColors[2];

                //if (this.Detected)
                if (kinectBodyView.kinectFeedback.currentFlexAngle <= 5.0f || kinectBodyView.kinectFeedback.currentFlexAngle >= -5.0f)
                {
                    //this.Confidence = detectionConfidence;
                    this.ImageSource = this.zeroImage;
                    this.BodyColor = this.trackedColors[2];
                }
                else
                {
                    this.ImageSource = this.notZeroImage;
