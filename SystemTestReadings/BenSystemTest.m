...
quaternionMid = zeros(length(tMid), 4);
for t = 1:length(tMid)
    AHRS.UpdateIMU(gyroMid(t,:) * (pi/180), accMid(t,:));	% gyroscope units must be radians
...
%SMid_plot_yaxis = downsample(SMid_plot_yaxis,round(length(SMid_plot_yaxis)/length(Vic_plot_yaxis)));
%SMid_plot_xaxis = downsample(SMid_plot_xaxis,round(length(SMid_plot_xaxis)/length(Vic_plot_xaxis)));
%x = 1:length(SMid_plot_yaxis)/length(Vic_plot_yaxis):length(SMid_plot_yaxis);
...
%xx = 1:length(Vic_plot_yaxis)/length(SMid_plot_yaxis):length(Vic_plot_yaxis);

%Vic_plot_yaxis = interp1(x,Vic_plot_yaxis);
%Vic_plot_xaxis = interp1(x,Vic_plot_xaxis);
...
%subplot(2,1,2)
plot(tMid, eulerMid(:,2), tMid, distanceMid(:,2))
legend('Madgwick','Unfiltered')
...
%difference = single(eulerMid(:,2)) - single(gyroMid(:,1));
%squaredError = difference .^ 2;
%meanSquaredError = sum(squaredError(:)) / numel(eulerMid(:,2));
%rmsError = sqrt(meanSquaredError);

RMSE_VicVsIMU=sqrt(mean(((Vic_plot_yaxis-SMid_plot_yaxis).^2)))
RMSE_Madgwick=sqrt(mean(((eulerMid(:,2)-gyroMid(:,1)).^2)))
