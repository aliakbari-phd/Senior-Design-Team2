...
SMidVelocity = diff(SMid_plot_yaxis);
SMidAcceleration = diff(SMidVelocity);
SMidJerk = diff(SMidAcceleration);
VicVelocity = diff(Vic_plot_yaxis);
VicAccel = diff(VicVelocity);
VicJerk = diff(VicAccel);
KinVelocity = diff(Kin_plot_y);
KinAccel = diff(KinVelocity);
KinJerk = diff(KinAccel);

SMidVelocity = resample(SMidVelocity, length(VicVelocity),length(SMidVelocity));
SMidAcceleration = resample(SMidAcceleration, length(VicAccel),length(SMidAcceleration));
SMidJerk = resample(SMidJerk, length(VicJerk),length(SMidJerk));
...
velRMSE_IMU = sqrt(mean((VicVelocity - SMidVelocity).^2))
accelRMSE_IMU = sqrt(mean((VicAccel - SMidAcceleration).^2))
jerkRMSE_IMU = sqrt(mean((VicJerk - SMidJerk).^2))

velRMSEPlot = sqrt((VicVelocity - SMidVelocity).^2);
accelRMSEPlot = sqrt((VicAccel - SMidAcceleration).^2);
jerkRMSEPlot = sqrt((VicJerk - SMidJerk).^2);

%velRMSE_plot_xaxis = resample(velRMSEPlot,length(Vic_plot_xaxis),length(SMid_plot_xaxis));
subplot(4,1,2)
plot(velRMSEPlot)
title('IMU RMSE Angular Velocity')
ylabel('RMSE (%)'),xlabel('Frame')
subplot(4,1,3)
plot(accelRMSEPlot)
title('IMU RMSE Angular Acceleration')
ylabel('RMSE (%)'),xlabel('Frame')
subplot(4,1,4)
plot(jerkRMSEPlot)
title('IMU RMSE Angular Jerk')
ylabel('RMSE (%)'),xlabel('Frame')